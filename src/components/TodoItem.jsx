    ...
    return (
        <li className="flex flex-col sm:flex-row sm: items-center justify-between border-b-2 py-2 gap-4">
            ...
            <div className="space-x-3 ml-8">
                ...
                {
                    !todo.completed && (<button onClick={() => dispatch(markCompleted(index))} className='mr-2 text-sm bg-green-500 text-white sm:px-2 py-1 px-1 rounded'>
                        {<FaCheck />}
                    </button>)
                }

                {
                    todo.completed && (<button onClick={() => dispatch(markIncompleted(index))} className='mr-2 text-sm bg-yellow-500 text-white sm:px-2 py-1 px-1 rounded'>
                        {<FaTimes/>}
                    </button>)
                }
            </div>
